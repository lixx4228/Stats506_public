...
p1 = plot_data %>% 
  ...
  ggplot(mapping = aes(x = division, y = est, color = rurality)) +
...
# ggsave(p1, 'w4_p2_q1_plot.png', device = 'png')
p1
...
p2 = plot_data %>% 
  filter(rurality == 'diff') %>% 
  ggplot(mapping = aes(x = division, y = est)) + geom_point() +
  geom_errorbar(aes(ymin = lwr, ymax = upr)) +
...
# Remove comments to plot and save
# ggsave(p2, 'w4_p2_q2_plot.png', device = 'png')
p2
