...
```{r q3}
orange %>% 
  mutate(z_score_age = (age - mean(age)) / sd(age))

```

 
4. Write a dplyr pipe to calculate the average rate of growth
(cm/year) across all trees between age 0 (assume circumference = 0) and
the final measured age for each tree.

```{r q4}
orange2 = orange %>% 
  group_by(Tree) %>% 
  mutate(growth = max(circumference) / max(age)) %>% 
  summarise(avg_rate_growth = mean(growth))
...
```r
 orange %>%
   group_by(Tree) %>%
   mutate( 
     new_growth = c(circumference[1], diff(circumference)), # difference of grouwth by row
     elapsed = c(age[1], diff(age)) # difference of age by row
   ) %>%
   group_by(age) %>% 
   summarize( 
...
This pipe generates result of 3 columns: age, avg_rate (average growth rate by age group), and se (sd of growth rate by age group); 7 rows of age group.

The results shows no obvious relationship between age and average growth rate. Growth rate for small age and large age don't differ much but it various a lot for age in between.
